...
const MobileNav = ({ showNav, setShowNav }: Props) => {
  ...
  return (
    <div id='wrapper' onClick={handleCloseModal} className={` ${navStyle}  fixed right-0 transition-all duration-500 left-0 top-0 bottom-0 h-[100vh] bg-[#000000e0] z-[1002]`}>
    ...
    <div className='relative bg-white transition-all duration-500 delay-200 flex flex-col items-center pt-[20%] w-[70%] h-[100%] overflow-auto'>
      ...
      <div className='flex items-center pt-4 space-x-4'>
          <button onClick={() => dispatch(toggleModalContactHomePage())}  className='px-6 py-2 sm:px-8 sm:py-3 text-[14px] sm:text-[16px] bg-blue-950 transition-all duration-200 hover:bg-red-600 flex items-center rounded-md space-x-2  text-white'>
              <span>
          ...
          <button onClick={() => dispatch(toggleModalRegisterHomePage())}   className='px-6 py-2 sm:px-8 sm:py-3 text-[14px] sm:text-[16px] bg-orange-500 transition-all duration-200 hover:bg-green-700 flex items-center rounded-md space-x-2  text-white'>
            <FaRegistered 
  className='w-[1.3rem] h-[1.3rem] sm:w-[1.7rem] sm:h-[1.7rem]'/>
          </button>
